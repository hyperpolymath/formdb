...
[.lead]
The principles and values that guide FormBD's design.
...
Traditional databases hide their internals. FormBD exposes them as readable, auditable artefacts.

== Value Hierarchy

FormBD explicitly prioritizes certain values over others:
...
FormBD will never be a drop-in PostgreSQL replacement.
...
FormBD exists because we believe:
